...
import { Mail, Calendar, Scale, Ruler, Phone, CreditCard, Heart } from 'lucide-react'
import { enrichProfileWithCalculations } from '@/lib/utils/health-calculations'
import { loginOrCreateUserWithProfile } from '@/lib/auth/auth-utils'
...
export function UserLoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Login or create user with profile data
      const result = await loginOrCreateUserWithProfile({
        ...enrichedProfile,
        sex: enrichedProfile.sex as 'male' | 'female',
        createdAt: enrichedProfile.createdAt || new Date().toISOString(),
      })
      ...
      if (result.success) {
        ...
        setError(result.error || 'Error al procesar la información')
      }
    } catch (error) {
      console.error('Login/create user error:', error)
      setError('Error al procesar la información. Por favor, inténtelo de nuevo.')
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-green-50 p-4">
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-slate-900 mb-2">Portal Paciente</h1>
          <p className="text-slate-600">
            Ingrese su información personal para acceder a su perfil de salud. Si es su primera vez,
            crearemos su perfil automáticamente.
          </p>
        </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="text-center sm:text-left">
                ...
                <p className="text-xs text-slate-600">
                  Si ya tienes una cuenta, actualizaremos tu información. Si no, crearemos tu
                  perfil.
                </p>
              </div>
              <Button
